...
func (m *Manager) rebuildAPIKeyModelAliasLocked(cfg *internalconfig.Config) {
	...
	for _, auth := range m.auths {
		...
		case "notion":
			if entry := resolveNotionAPIKeyConfig(cfg, auth); entry != nil {
				compileAPIKeyModelAliasForModels(byAlias, entry.Models)
			}
		default:
...
func (m *Manager) applyAPIKeyModelAlias(auth *Auth, requestedModel string) string {
	...
	case "notion":
		upstreamModel = resolveUpstreamModelForNotionAPIKey(cfg, auth, requestedModel)
	default:
...
func resolveNotionAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.NotionKey {
	if cfg == nil {
		return nil
	}
	return resolveAPIKeyConfig(cfg.NotionKey, auth)
}
...
func resolveUpstreamModelForNotionAPIKey(cfg *internalconfig.Config, auth *Auth, requestedModel string) string {
	entry := resolveNotionAPIKeyConfig(cfg, auth)
	if entry == nil {
		return ""
	}
	return resolveModelAliasFromConfigModels(requestedModel, asModelAliasEntries(entry.Models))
