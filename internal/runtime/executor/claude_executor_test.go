...
func TestCheckSystemInstructionsWithMode_StringSystemPreserved(t *testing.T) {
	...
	if got := blocks[2].Get("text").String(); !strings.Contains(got, "You are an interactive agent that helps users with software engineering tasks.") {
		t.Fatalf("blocks[2] should be static Claude Code prompt, got %q", got)
	}
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "You are a helpful assistant.") || !strings.Contains(got, "hi") {
		t.Fatalf("user system prompt should be forwarded into first user message, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_EmptyStringSystemIgnored(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); got != "hi" {
		t.Fatalf("empty string system should not alter first user message, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_ArraySystemStillWorks(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "Be concise.") {
		t.Fatalf("array system prompt should be forwarded into first user message, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_StringWithSpecialChars(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, `Use <xml> tags & "quotes" in output.`) {
		t.Fatalf("forwarded user system prompt text mangled, got %q", got)
	}
}
...
func TestApplyCloaking_PreservesConfiguredStrictModeAndSensitiveWordsWhenModeOmitted(t *testing.T) {
	...
	blocks := gjson.GetBytes(out, "system").Array()
	if len(blocks) != 3 {
		t.Fatalf("expected strict mode to keep 3 injected system blocks, got %d", len(blocks))
	}
	if got := gjson.GetBytes(out, "messages.0.content.0.text").String(); strings.Contains(got, "system-reminder") {
		t.Fatalf("strict mode should not forward original system prompt into message content, got %q", got)
	}
