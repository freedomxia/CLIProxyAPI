...
一个为 CLI 提供 OpenAI/Gemini/Claude/Codex/Notion 兼容 API 接口的代理服务器。
...
- 支持原生 Notion AI provider（`notion-api-key` / token_v2）
...
- 原生 Notion Provider：查看 `config.example.yaml` 中的 `notion-api-key` 示例；如果你想把凭证落成 `auths/*.json`，管理面板可直接进入 `Auth Files -> 新增 Notion 凭证`
- Notion2API Sidecar 集成: [NOTION2API_INTEGRATION_CN.md](NOTION2API_INTEGRATION_CN.md)
